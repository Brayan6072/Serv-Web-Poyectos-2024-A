                 ...
                 try {
                      List<Usuarios> lista = AccionUsuarios.ConsultarUsuarios();
                            for(Usuarios us : lista){
                               correo.sendEmail(us.getEmail(), "Reporte de " + etiqueta, "<h1>Hola</h1>"+"Bote de "+tpb+" se encuentra "+estado);
                            }
                } catch (MessagingException e) {
                    e.printStackTrace();
                }
                 
                response.sendRedirect("http://mapeoresiduos.jelastic.saveincloud.net/"); 
